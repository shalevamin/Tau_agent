...
export function renderOverview(props: OverviewProps) {
  ...
  const capabilityCards = [
    {
      title: "OpenAI operator stack",
      value: "Codex + GPT-5.4 CUA",
      detail: "Tau seeds the OpenAI CUA runner locally and exposes it through tua-cua-dev, tua-cua-runner, and tua-cua-web.",
    },
    {
      title: "Agent orchestra",
      value: `${props.agentsCount} agents`,
      detail: "Subagent depth and aggressive conductor defaults are seeded at install time.",
    },
    {
      title: "Installed skill mesh",
      value: `${props.skillsCount} skills`,
      detail: "OpenAI, browser-use, BrowserMCP, Superset, and Tua skills are loaded together behind one operator layer.",
    },
    {
      title: "Tool catalog",
      value: `${props.toolsCount} tools`,
      detail: "Coding, browsing, system control, messaging, docs, and orchestration live behind one OpenAI-first operator surface.",
    },
    {
      title: "Nodes + devices",
      value: `${props.nodesCount} nodes / ${props.pendingDevices} pending`,
      detail: "Paired machines, approvals, and device control stay visible from one dashboard.",
    },
    {
      title: "Native browser stack",
      value: `${props.connectedChannels} connected channels`,
      detail: "Browser-use comes first, computer-use is second, and relay-style browser takeover stays as fallback.",
    },
  ];
  const quickActions: Array<{ tab: Tab; label: string }> = [
    { tab: "chat", label: "Chat" },
  ...
  return html`
    ...
    <section class="grid grid-cols-2" style="margin-top: 18px;">
      <div class="card">
        <div class="card-title">Tua Command Center</div>
        <div class="card-sub">Jump straight into the surface that matches the job.</div>
        <div class="tua-action-grid" style="margin-top: 16px;">
          ${quickActions.map(
            (action) => html`
              <button class="btn btn--sm" @click=${() => props.onOpenTab(action.tab)}>${action.label}</button>
            `,
          )}
        </div>
        <div class="callout" style="margin-top: 14px;">
          Tau is OpenAI-only: Codex and the local OpenAI CUA stack lead first, browser-use is the default web lane,
          computer-use is second, and extension relay stays fallback-only. Logged-in app sessions still beat asking
          for extra SaaS keys.
        </div>
        <div class="callout" style="margin-top: 12px;">
          Local operator console: <span class="mono">~/.openclaw/tools/openai-cua-sample-app</span><br />
          Launch commands: <span class="mono">tua-cua-dev</span>, <span class="mono">tua-cua-runner</span>,
          <span class="mono">tua-cua-web</span>
        </div>
      </div>

      <div class="card">
        <div class="card-title">Tua Capability Mesh</div>
        <div class="card-sub">Everything the install seeds into the operator stack.</div>
        <div class="tua-capability-grid" style="margin-top: 16px;">
          ${capabilityCards.map(
            (card) => html`
              <div class="tua-capability-card">
                <div class="note-title">${card.title}</div>
                <div class="stat-value" style="margin-top: 6px;">${card.value}</div>
