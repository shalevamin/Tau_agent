...
export async function onboardCommand(opts: OnboardOptions, runtime: RuntimeEnv = defaultRuntime) {
  ...
  if (
    normalizedOpts.authChoice &&
    !TAU_ALLOWED_AUTH_CHOICES.has(normalizedOpts.authChoice)
  ) {
    runtime.error(
      [
        "Tau is configured for OpenAI-only onboarding.",
        `Unsupported auth choice: ${normalizedOpts.authChoice}`,
        'Use "--auth-choice openai-api-key" or "--auth-choice openai-codex".',
      ].join("\n"),
    );
    runtime.exit(1);
    return;
