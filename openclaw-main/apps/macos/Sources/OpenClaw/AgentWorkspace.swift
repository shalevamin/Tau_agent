...
enum AgentWorkspace {
    ...
    static let dailyMemoryDirname = "memory"
    ...
    private static let templateEntries: Set<String> = [
        ...
        AgentWorkspace.dailyMemoryDirname,
    ...
    static func bootstrap(workspaceURL: URL) throws -> URL {
        ...
        try self.ensureMemoryScaffold(workspaceURL: workspaceURL)
    ...
    static func ensureMemoryScaffold(workspaceURL: URL) throws {
        ...
        let dailyMemoryURL = workspaceURL.appendingPathComponent(self.dailyMemoryDirname, isDirectory: true)
        try FileManager().createDirectory(at: dailyMemoryURL, withIntermediateDirectories: true)

        let todayURL = dailyMemoryURL.appendingPathComponent(self.todayMemoryFilename())
        if !FileManager().fileExists(atPath: todayURL.path) {
            try self.defaultTodayMemoryTemplate().write(to: todayURL, atomically: true, encoding: .utf8)
            self.logger.info("Created daily memory note at \(todayURL.path, privacy: .public)")
        }
    }
    ...
    static func defaultTemplate() -> String {
        ...
        ## Tua routing defaults
        - Prefer orchestration, coding-agent flows, `codex`, and the local OpenAI operator stack (`tua-cua-runner`, `tua-cua-web`, `tua-cua-dev`) for non-trivial coding and browser work.
        - Prefer browser automation in this order: `browser-use` first, then computer-use on the real UI, and only then extension relay or other browser takeover paths.
        - For docs, use real doc workflows (browser, Google Docs, Word, doc skills) when available.
        - Use OpenAI-family models as the only model provider. Reuse browser sessions instead of routing work to other LLM vendors.
        - Install missing local tools when approvals allow and it materially helps complete the task.

        ## Daily memory (recommended)
        - Keep a short daily log at memory/YYYY-MM-DD.md (create memory/ if needed).
        - On session start, read today + yesterday if present.
        - Write stable user facts, preferences, accounts, devices, workflows, and project context into MEMORY.md as soon as you learn them.
        - Write volatile session context, discoveries, and work logs into today's memory file immediately instead of trusting conversational context to survive.
        - If the user says "remember this", persist it to disk in the appropriate memory file during the same turn.
    ...
    static func defaultMemoryTemplate() -> String {
        ...
        Durable facts belong here. Keep it concise, current, and useful.
        ...
        - Key services the user actively uses:
        - Preferred document stack:
        ...
        ## Durable Notes
        - Record stable decisions, recurring workflows, and facts worth remembering across sessions.
        """
    ...
    private static func todayMemoryFilename(now: Date = .init()) -> String {
    ...
    private static func defaultTodayMemoryTemplate(now: Date = .init()) -> String {
        let formatter = DateFormatter()
        formatter.calendar = Calendar(identifier: .gregorian)
        formatter.locale = Locale(identifier: "en_US_POSIX")
        formatter.timeZone = .current
