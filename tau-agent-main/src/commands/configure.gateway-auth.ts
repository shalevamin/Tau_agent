...
export async function promptAuthConfig(
  ...
  if (authChoice === "custom-api-key") {
    ...
    const modelSelection = await promptDefaultModel({
      ...
      preferredProvider:
        authChoice === "skip" ? "openai" : resolvePreferredProviderForAuthChoice(authChoice),
    });
  ...
  if (authChoice !== "custom-api-key") {
    const allowlistSelection = await promptModelAllowlist({
      config: next,
      prompter,
      allowedKeys: anthropicOAuth ? ANTHROPIC_OAUTH_MODEL_KEYS : TAU_OPENAI_MODEL_KEYS,
      initialSelections: anthropicOAuth ? ["anthropic/claude-sonnet-4-6"] : [TAU_OPENAI_MODEL_KEYS[0]],
      message: anthropicOAuth ? "Anthropic OAuth models" : "OpenAI models in /model picker",
    });
