...
function countChannelAccounts(
  snapshot: AppViewState["channelsSnapshot"],
  predicate: (account: { configured?: boolean | null; connected?: boolean | null }) => boolean,
): number {
  const accounts = Object.values(snapshot?.channelAccounts ?? {}).flat();
  return accounts.filter(predicate).length;
}

function countCatalogTools(result: AppViewState["toolsCatalogResult"]): number {
  return (result?.groups ?? []).reduce((sum, group) => sum + group.tools.length, 0);
}
...
export function renderApp(state: AppViewState) {
  ...
  const configuredChannels = countChannelAccounts(
    state.channelsSnapshot,
    (account) => account.configured === true,
  );
  const connectedChannels = countChannelAccounts(
    state.channelsSnapshot,
    (account) => account.connected === true,
  );
  const agentsCount = state.agentsList?.agents?.length ?? 0;
  const skillsCount = state.skillsReport?.skills?.length ?? 0;
  const nodesCount = state.nodes.length;
  const pendingDevices = state.devicesList?.pending?.length ?? 0;
  const toolsCount = countCatalogTools(state.toolsCatalogResult);
        ...
        <div class="topbar-left">
          ...
          <div class="brand">
            <div class="brand-logo">
              <img src=${basePath ? `${basePath}/favicon.svg` : "/favicon.svg"} alt="Tua Agent" />
            </div>
            <div class="brand-text">
              <div class="brand-title">TUA AGENT</div>
              <div class="brand-sub">Ultimate Operator Dashboard</div>
            </div>
          </div>
        </div>
